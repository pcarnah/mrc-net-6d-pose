...
if __name__ == '__main__':
    ...
    dp_model = dataset_params.get_model_params(
        p['bop_root'], 'usprobe', model_type)
    dp_data = dataset_params.get_split_params(
        p['bop_root'],'usprobe', 'test')
    with open(dp_model['models_info_path'], 'r') as fp:
        model_info = json.load(fp)

    dp_data['split_path'] = os.path.join(p['bop_root'], p['dataset'], bop_cfg.DATASET_CONFIG[p['dataset']]['test_set'][0])
    ...
    test_path = Path(dp_data['split_path'])
    for sc in test_path.iterdir():
        scene_id = int(sc.name)

        with open(sc.joinpath('scene_gt.json'), 'r') as f:
            scene_gt = json.load(f)

        with open(sc.joinpath('scene_gt_info.json'), 'r') as f:
            scene_gt_info = json.load(f)

        for view_id in tqdm(scene_gt):
            for i, (gt, info) in enumerate(zip(scene_gt[view_id], scene_gt_info[view_id])):
                view_id = int(view_id)
