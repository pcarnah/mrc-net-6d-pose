...
def get_affine_matrix(center, scale, rot, output_size, camK):
    ...
    T_rot = camK @ Rz @ torch.linalg.inv(camK)
    center_hom = T_rot @ torch.tensor([*center, 1.0], dtype=torch.float32)
    center = center_hom[:2]
    T_img2roi = transform_to_local_ROIcrop(center, scale, output_size[0])
    trans = T_img2roi @ T_rot
    return trans[:2]

def crop_resize_by_warp_affine(img, center, scale, output_size, camK, rot=0, interpolation='bilinear', debug=False):
    """
    output_size: int or (w, h)
    ...
    trans = get_affine_matrix(center, scale, rot, output_size, camK)
    if debug:
...
def aug_bbox_DZI(bbox_xyxy, im_H, im_W, scale_ratio, shift_ratio, pad_scale, dyn_pad=False):
    ...
    if dyn_pad:
        ...
        scale_ratio = scale_ratio * dyn_pad_scale
    else:
        scale_ratio = scale_ratio * pad_scale
    scale = max(y2 - y1, x2 - x1) * scale_ratio
    scale = min(scale, max(im_H, im_W)) * 1.0
    bh_loc = bh / scale
    bw_loc = bw / scale
    bbox_loc = np.array([0.5 - bw_loc/2, 0.5 - bh_loc/2, 0.5 + bw_loc/2, 0.5 + bh_loc/2])
    return bbox_center, scale, bbox_loc
...
def transform_to_local_ROIcrop(bbox_center, bbox_scale, zoom_scale=256):
    ...
    Ts_B2X = list()
    bbox_topleft = bbox_center - bbox_scale / 2
    for bxby, bs in zip(bbox_topleft, bbox_scale):
        r = zoom_scale / bs
        T_b2x = torch.tensor([
            [r, 0, -r * bxby[0]], 
            [0, r, -r * bxby[1]], 
