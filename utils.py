...
# Bumping this invalidates every cached quaternion-bin label array
# (see ``bop_dataset.py``): it identifies the prototype grid generated by
# ``make_quaternion_prototypes`` below.
PROTOTYPE_VERSION = 1
PROTOTYPE_N_SIDE = 4
...
def make_quaternion_prototypes(device='cpu', n_side=PROTOTYPE_N_SIDE):
    """
...
def quantize_quaternion_vertex(quaternions, vertices, vertices_mask, diameter,
    ...
    quat_matched, trans_matched = top_matched_pose(
        ...
        prototypes_rep.reshape(-1, 4), trans_rep.view(-1, 3),
        v_corr_rep.view(-1, 3, 3), q_sym_rep.view(-1, num_sym, 4),
...
@torch.no_grad()
def top_matched_pose(q_src, t_src, q_tgt, t_tgt, v_corr,
